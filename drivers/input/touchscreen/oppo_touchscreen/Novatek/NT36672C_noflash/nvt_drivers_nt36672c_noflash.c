...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    ...
    if(chip_info->ts->dev_pm_suspend){
        TPD_INFO("chip_info->ts->dev_pm_suspend enable\n");
        ret = wait_for_completion_timeout(&chip_info->ts->dev_pm_resume_complete,msecs_to_jiffies(500));
        if(!ret){
            TPD_INFO("system (SPI) has not complete the process,skip it!\n");
            return -1;
    }
    }
#endif
...
int __maybe_unused nvt_tp_probe(struct spi_device *client)
{
	...
	init_completion(&ts->dev_pm_resume_complete);
#endif
...
static int nvt_spi_suspend(struct device *dev)
{
	...
	reinit_completion(&ts->dev_pm_resume_complete);
#endif
    tp_i2c_suspend(ts);
...
#if NVT_PM_WAIT_SPI_RESUME_COMPLETION
	ts->dev_pm_suspend = false;
	complete(&ts->dev_pm_resume_complete);
#endif
    tp_i2c_resume(ts);
